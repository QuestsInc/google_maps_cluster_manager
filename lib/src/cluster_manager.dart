...
class ClusterManager<T extends ClusterItem> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<pi.Marker> Function(Cluster<T>)? markerBuilder,
  ...
  final Future<pi.Marker> Function(Cluster<T>) markerBuilder;
  ...
  final void Function(Set<pi.Marker>) updateMarkers;
  ...
  void setMapId(int mapId, {bool withUpdate = true}) async {
    _mapId = mapId;
    _zoom = await pi.GoogleMapsFlutterPlatform.instance.getZoomLevel(mapId: mapId);
  ...
  void _updateClusters() async {
    ...
    final Set<pi.Marker> markers = Set.from(await Future.wait(mapMarkers.map((m) => markerBuilder(m))));
  ...
  void onCameraMove(pi.CameraPosition position, {forceUpdate = false}) {
    _zoom = position.zoom;
    if (forceUpdate) {
      updateMap();
    }
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    final pi.LatLngBounds mapBounds = await pi.GoogleMapsFlutterPlatform.instance.getVisibleRegion(mapId: _mapId!);

    late pi.LatLngBounds inflatedBounds;
  ...
  pi.LatLngBounds _inflateBounds(pi.LatLngBounds bounds) {
    ...
    return pi.LatLngBounds(
      southwest: pi.LatLng(bounds.southwest.latitude - lat, wLng),
      northeast: pi.LatLng(bounds.northeast.latitude + lat, lng != 0 ? eLng : _maxLng),
    );
  }
  ...
  static Future<pi.Marker> Function(Cluster) get _basicMarkerBuilder => (cluster) async {
        return pi.Marker(
          markerId: pi.MarkerId(cluster.getId()),
  ...
  static Future<pi.BitmapDescriptor> _getBasicClusterBitmap(int size, {String? text}) async {
    ...
    return pi.BitmapDescriptor.fromBytes(data.buffer.asUint8List());
  }
}
